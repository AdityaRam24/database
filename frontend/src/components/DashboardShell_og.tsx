...
import { LogOut, LogIn, Zap, GitMerge, Database, BookOpen, Activity, Shield, ShieldAlert, Bot } from "lucide-react";
...
const NAV_ITEMS = [
    ...
    { path: '/dashboard/performance', label: 'Speed Optimization', icon: Zap, color: '#f59e0b', textColor: '#fbbf24' },
    { path: '/dashboard/governance', label: 'Safe Changes', icon: GitMerge, color: '#3b82f6', textColor: '#60a5fa' },
    { path: '/dashboard/data', label: 'Data Explorer', icon: Database, color: '#10b981', textColor: '#34d399' },
    { path: '/dashboard/semantic', label: 'Company Knowledge', icon: BookOpen, color: '#a78bfa', textColor: '#c4b5fd' },
    { path: '/dashboard/anomaly', label: 'Vital Signs', icon: Activity, color: '#ef4444', textColor: '#f87171' },
    { path: '/dashboard/incidents', label: 'Alerts', icon: ShieldAlert, color: '#f97316', textColor: '#fb923c' },
    { path: '/dashboard/security', label: 'Privacy & Security', icon: Shield, color: '#818cf8', textColor: '#a5b4fc' },
            ...
            <div className="flex w-full h-full flex-1">
                ...
                <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
                    ...
                    <header className="sticky top-3 z-40 w-full px-3 sm:px-4">
                        <nav className="flex items-center justify-between px-3 sm:px-4 py-2 bg-white/90 backdrop-blur-2xl border border-black/8 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.10)]">
                            {/* Logo */}
                            <div className="flex items-center gap-2 shrink-0">
                                <span className="w-7 h-7 rounded-lg bg-gradient-to-br from-violet-500 to-purple-700 flex items-center justify-center shadow-md shadow-violet-500/20">
                                    <Database size={13} className="text-white" strokeWidth={2.5} />
                                </span>
                                <div className="hidden sm:block">
                                    <h1 className="text-slate-800 font-bold text-sm whitespace-nowrap leading-tight">DB-Lighthouse</h1>
                                    {projectName && <p className="text-slate-500 text-[9px] uppercase tracking-wider">{projectName}</p>}
                                </div>
                            </div>

                            {/* Desktop Links */}
                            <div className="hidden xl:flex flex-1 justify-center relative px-2 min-w-0">
                                <NavBar items={NAV_ITEMS.map(item => ({ name: item.label, url: item.path, icon: item.icon, color: item.color, textColor: item.textColor }))} className="w-full" />
                            </div>
                                    ...
                                    <SheetContent side="right" className="bg-white/95 dark:bg-[#0b0b14]/95 border-l border-black/5 dark:border-white/5 backdrop-blur-xl sm:max-w-sm w-3/4 p-0">
                                        <div className="flex flex-col h-full bg-transparent">
                                            <div className="p-6 border-b border-black/5 dark:border-white/5 flex items-center gap-3">
                                                <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-purple-700 flex items-center justify-center shadow-md shadow-violet-500/20">
                    ...
                    {/* Body Content */}
                    <div style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column' }} className="mt-8 px-2 sm:px-4">
                        {children}
                    </div>
                </div>
            </div>
