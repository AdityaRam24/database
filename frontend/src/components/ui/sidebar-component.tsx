    ...
    <div className="aspect-[24/24] grow min-h-px min-w-px overflow-clip relative shrink-0">
      <div className="absolute aspect-[24/16] left-0 right-0 top-1/2 -translate-y-1/2">
        <svg className="block size-full" fill="none" viewBox="0 0 24 16">
          <g>
            <path d={svgPaths.p36880f80} fill="#8b5cf6" />
            <path d={svgPaths.p355df480} fill="#8b5cf6" />
            <path d={svgPaths.pfa0d600} fill="#8b5cf6" />
          </g>
        </svg>
      </div>
    </div>
...
function ProjectDeleteAction({ project, onDeleted }: { project: Project, onDeleted: () => void }) {
  const { user } = useAuth();
    ...
    return (
      <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
        <button
          ...
          disabled={deleting}
          className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-md bg-red-500/20 text-red-400 hover:bg-red-500/30 border border-red-500/30 transition-colors"
        >
          {deleting ? "—" : "Delete"}
        </button>
        <button
    ...
    dashboard: {
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            {
              icon: <AddLarge size={16} className="text-emerald-400" />,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {item.hasDropdown && (
          <div
            className={`flex items-center justify-center shrink-0 transition-opacity duration-500 ${isCollapsed ? "opacity-0 w-0" : "opacity-100"
              }`}
            style={{ transitionTimingFunction: softSpringEasing }}
          >
            <ChevronDownIcon
        ...
        {item.action && !isCollapsed && (
          <div className="flex items-center justify-center shrink-0 z-10" onClick={e => e.stopPropagation()}>
            {item.action}
          </div>
        )}
      </div>
