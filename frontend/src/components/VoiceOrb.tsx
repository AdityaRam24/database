...
export default function VoiceOrb() {
  ...
  // Keep handleQuery ref always fresh to avoid stale closures in MediaRecorder callbacks
  useEffect(() => { handleQueryRef.current = handleQuery; }, [handleQuery]);

  // ── WAV encoder: converts raw Float32 PCM into a proper WAV file ──
  ...
  // ── Upload recorded audio to backend for server-side transcription ──
  const transcribeViaBackend = async (audioBlob: Blob) => {
    try {
      setTranscript('Transcribing via Jarvis Neural Core...');
      ...
      if (data.error) {
        setResult({ sql: '', rows: [], columns: [], chart_type: null, explanation: null, error: data.error });
        speak('I could not understand the audio, Sir.');
        return;
      }

      if (data.text && data.text.trim().length > 0) {
        setTranscript(data.text);
        handleQueryRef.current(data.text);
      } else {
        setTranscript('');
        speak('I didn\'t catch that, Sir. Please try again.');
      }
    } catch (err: any) {
      setResult({ sql: '', rows: [], columns: [], chart_type: null, explanation: null, error: `Backend transcription failed: ${err.message}` });
    }
  };
  ...
  const startListening = useCallback(async () => {
    // Resume speech synthesis just in case we are stuck
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        // Convert recorded chunks → WAV → upload to backend
        if (audioChunksRef.current.length > 0) {
          const webmBlob = new Blob(audioChunksRef.current, { type: 'audio/webm' });
          // Decode WebM to raw PCM via Web Audio API, then re-encode as WAV
          try {
            ...
            const wavBlob = encodeWAV(resampled.getChannelData(0), 16000);
            await audioCtx.close();
            await transcribeViaBackend(wavBlob);
          } catch (decodeErr: any) {
            setResult({ sql: '', rows: [], columns: [], chart_type: null, explanation: null, error: `Audio encoding failed: ${decodeErr.message}` });
          }
        }
        audioChunksRef.current = [];
      };
      ...
      setListening(true);
      playSound('start');
      setTranscript('Listening... click the mic again to stop');
      setResult(null);
      setManualInput('');
      startSimulatedVisualizer();

    } catch (e: any) {
      setResult({ sql: '', rows: [], columns: [], chart_type: null, explanation: null, error: 'Microphone access denied. Please allow microphone permissions in your browser.' });
      setListening(false);
      stopMic();
    }
  }, [stopMic, startSimulatedVisualizer, playSound]);
