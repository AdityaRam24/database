...
export default function VoiceOrb() {
  ...
  return (
    <>
      ...
      {open && pos.x > -100 && (() => {
        const PANEL_W = 360;
        const PANEL_H = 460; // approx max height
        const GAP = 12;
        ...
        return (
          <div
            className="fixed z-[9999] w-[360px] max-w-[calc(100vw-2.5rem)] rounded-3xl shadow-2xl overflow-hidden flex flex-col"
            style={{
              left: panelLeft, top: panelTop,
              background: 'rgba(8,8,18,0.97)', border: '1px solid rgba(139,92,246,0.35)',
              backdropFilter: 'blur(24px)', maxHeight: '70vh',
            }}
          >
            {/* Header */}
      ...
      {pos.x > -100 && (
        ...
        >
          {/* Drag hint tooltip */}
          <div className="absolute -top-6 left-1/2 -translate-x-1/2 pointer-events-none opacity-0 hover:opacity-100 transition-opacity whitespace-nowrap">
            <span className="text-[9px] text-slate-500 font-bold">drag to move</span>
          </div>
